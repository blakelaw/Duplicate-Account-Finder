...
#Creates dataframe of closest match for user based on distance metric
def process_linkage(CX, LBX):
    n_data_points = len(LBX)
    ...
    pairs_df['User1'] = pairs_df['User1'].apply(lambda x: LBX[x])
    pairs_df['User2'] = pairs_df['User2'].apply(lambda x: LBX[x])
    pairs_df = pairs_df.sort_values(by='Distance', ascending=True)
    
    return pairs_df

#Method 1: Analysis with top 50 word frequencies, filtered to only active users
...
#Hiearchical cluster analysis with taxicab metric and single-linkage
...
#Method 2: Analysis with time and top 50 word frequencies
...
#Method 3: Analysis with top 50 word frequencies
...
#Method 4: Analysis with syntax, timing, and word frequency
...
#Method 5: Analysis with syntax and word frequency
